...
switch (playerSide) do
{
	...
	case west:
	{
		...
		//IR Laser Add
		life_actions = life_actions + [player addAction["Add IR Laser (Non-Lethal)",life_fnc_mkDowning,"add",1.5,false,true,"",'"vehicle player != player && acc_pointer_IR" in (items player) && !("acc_pointer_IR" in (primaryWeaponItems player)) && (currentWeapon player) == "srifle_DMR_03_F"']];

		//IR Laser Remove
		life_actions = life_actions + [player addAction["<t color='#FF0000'>Remove IR Laser (Lethal)</t>",life_fnc_mkDowning,"rem",1.5,false,true,"",'vehicle player != player && !("acc_pointer_IR" in (items player)) && ("acc_pointer_IR" in (primaryWeaponItems player)) && (currentWeapon player) == "srifle_DMR_03_F"']];
	};
