...
if(_chance == 0) then
{
	...
	// JESSE - NEEDS FIXING  [[1,format["ALARM! - A bank being robbed by %1!",name player]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
};
...
if([true,_item,_qty] call life_fnc_handleInv) then {
	...
	// JESSE - NEEDS FIXING [[getPlayerUID _robber,name _robber,"458"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
};
